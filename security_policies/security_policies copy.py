...
def patch_one_rule_new(project_name, policy_name, rule_priority, body):
    print body
    try:
        patched_rule = security_policy_service.securityPolicies().patchRule(
            project=project_name, securityPolicy=policy_name, body=body, priority=rule_priority).execute()
